...
def _bundle_to_batch_ids_map(bundle_ids):
    """Return {bundle_name: 'C1234, C5678'} using SBE -> Batch(batch_id)."""
    if not bundle_ids:
        return {}
    ...
    batch_nos = list({r.batch_no for r in sbe_rows if r.batch_no})
    if not batch_nos:
        return {}
    ...
    batch_map = {b.name: b.batch_id for b in batch_rows}

    out_sets = {}
    for r in sbe_rows:
        bid = batch_map.get(r.batch_no)
        if bid:
            out_sets.setdefault(r.parent, set()).add(bid)

    # to comma-separated string
    return {k: ", ".join(sorted(v)) for k, v in out_sets.items()}
...
def execute(filters=None):
    ...
    # Fill batch_id for rows created via Serial & Batch Bundle (when sle.batch_no is empty)
    missing_bundles = [
        d.serial_and_batch_bundle
        for d in sl_entries
        if not d.get("batch_id") and d.get("serial_and_batch_bundle")
    ]
    if missing_bundles:
        bmap = _bundle_to_batch_ids_map(missing_bundles)
        for d in sl_entries:
            if not d.get("batch_id") and d.get("serial_and_batch_bundle"):
                d["batch_id"] = bmap.get(d.serial_and_batch_bundle)
...
def get_segregated_bundle_entries(sle, bundle_details, batch_balance_dict):
	...
	for row in bundle_details:
		...
		new_sle.update(row)
		new_sle["batch_id"] = row.get("batch_id")
		new_sle.update(
			{
...
def get_serial_batch_bundle_details(sl_entries, filters=None):
    ...
    # 👇 NEW: fetch batch_id for all batch_nos in one go
    batch_nos = list({e.batch_no for e in batch_entries if e.batch_no})
    batch_map = {}
    if batch_nos:
        for d in frappe.get_all("Batch", filters={"name": ("in", batch_nos)}, fields=["name", "batch_id"]):
            batch_map[d.name] = d.batch_id

    for entry in batch_entries:
        entry.batch_id = batch_map.get(entry.batch_no)  # 👈 NEW
